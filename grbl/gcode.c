...
uint8_t gc_execute_line(char *line)
{
	...
	uint8_t axis_words = 0; // XYZ tracking (also ABC if enabled)
...
//	uint16_t value_words = 0; // Tracks value words.
	...
	{ // Loop until no more g-code words in line.
		...
		default:
			...
			case 'P':
				...
				break;
				// NOTE:
				//  for digital controls M62,M63, P value is an integer
			case 'Q':
				word_bit = WORD_Q;
			...
			if ( bit(word_bit)
					& (bit(WORD_F) | bit(WORD_N) | bit(WORD_P) | bit(WORD_T) | bit(WORD_S)))
			{
				if (value < 0.0)
				{
	...
	if (axis_command)
	{
		bit_false(value_words,
				(bit(WORD_X)|bit(WORD_Y)|bit(WORD_Z)|bit(WORD_A)|bit(WORD_B)|bit(WORD_C)));
	} // Remove axis words.
	...
	if (gc_block.modal.digital)
	{
		...
		{
			FAIL(STATUS_GCODE_VALUE_WORD_MISSING);
		} // [P word missing] need P value for M62,M63
		bit_false(value_words, bit(WORD_P));
	}
	if (gc_block.modal.waitoninput)
	{
		...
		{
			FAIL(STATUS_GCODE_VALUE_WORD_MISSING);
		} // [P or L or Q word missing] need E P Q values for M66
	...
	if (gc_block.modal.analog)
	{
		...
		{
			FAIL(STATUS_GCODE_VALUE_WORD_MISSING);
		} // [E or Q word missing] need E and Q value for M67
	...
	if (gc_block.modal.accel_scaling)
	{
		...
		{
			FAIL(STATUS_GCODE_VALUE_WORD_MISSING);
		} // [P or Q missing] need P Q values for M100
		...
		if (axis_command == AXIS_COMMAND_MOTION_MODE)
		{
					...
					|| (gc_state.modal.motion == MOTION_MODE_CCW_ARC))
			{
				mc_arc(gc_block.values.xyz, pl_data, gc_state.position,
						gc_block.values.ijk, gc_block.values.r, axis_0, axis_1, axis_linear,
