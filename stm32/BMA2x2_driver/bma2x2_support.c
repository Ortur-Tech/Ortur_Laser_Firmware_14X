...
/*----------------------------------------------------------------------------*
*  V_BMA2x2RESOLUTION_U8 used for selecting the accelerometer resolution
 *	12 bit
 *	14 bit
 *	10 bit
*----------------------------------------------------------------------------*/
...
s32 bma2x2_initialization(void)
{
	...
	s32 com_rslt = ERROR;
	//加速度计测量范围
	u8 bma2x2_range = BMA2x2_RANGE_2G;
    u8 bma2x2_range_xg = 0;
 ...
 *-------------------------------------------------------------------------*/
    ...
    if(bma2x2_range == BMA2x2_RANGE_2G)
    	bma2x2_range_xg = 2;
    if(bma2x2_range == BMA2x2_RANGE_4G)
        bma2x2_range_xg = 4;
    if(bma2x2_range == BMA2x2_RANGE_8G)
        bma2x2_range_xg = 8;
    if(bma2x2_range == BMA2x2_RANGE_16G)
        bma2x2_range_xg = 16;

	switch (bma2x2.chip_id)
	{
		case 0x03:
...
s32 bma2x2_1de_initialization(void)
{
	/* status of communication*/
		s32 com_rslt = ERROR;
	/*-----------------------------------------------------------------------*
...
s32 bma2x2_data_readout(s16 * accel_x_s16,s16 * accel_y_s16,s16 * accel_z_s16)
{
	...
	if(bma2x2_inited==0)
	{
		com_rslt = bma2x2_initialization();
		if(com_rslt==SUCCESS)
			bma2x2_inited = 1;
		if(bma2x2_inited==0)
			return com_rslt;
	...
	*---------------------------------------------------------------------*/
	...
	com_rslt += bma2x2_read_accel_x(accel_x_s16); *accel_x_s16 = *accel_x_s16/sensitivity*1000;
	/* Read the accel Y data*/
	com_rslt += bma2x2_read_accel_y(accel_y_s16); *accel_y_s16 = *accel_y_s16/sensitivity*1000;
	/* Read the accel Z data*/
	com_rslt += bma2x2_read_accel_z(accel_z_s16); *accel_z_s16 = *accel_z_s16/sensitivity*1000;
...
//	sprintf(buf,"accel:y = %d.\r\n",sample_xyzt.y);
//	sprintf(buf,"accel:z = %d.\r\n",sample_xyzt.z);
//	sprintf(buf,"tempe:t = %d.\r\n",sample_xyzt.temp);
//	debugStr(buf);
...
s8 BMA2x2_I2C_bus_read(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
	...
	for(j=0;j<cnt;j++)
	{
		if(j==(cnt-1))
		{
			array[j]=IIC_Read_Byte(0);
		}
		else
		{
			array[j]=IIC_Read_Byte(1);
		}
	}
	//IIC_Ack();
	IIC_Stop();
