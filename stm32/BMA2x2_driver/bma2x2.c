...
u8 *data_u8, u32 len_u32)
{
	...
	BMA2x2_RETURN_FUNCTION_TYPE com_rslt = BMA_ERROR;
...
BMA2x2_RETURN_FUNCTION_TYPE bma2x2_init(struct bma2x2_t *bma2x2)
{
	...
	if (p_bma2x2 == BMA2x2_NULL) {
		...
		/* read Chip Id */
		com_rslt = p_bma2x2->BMA2x2_BUS_READ_FUNC(p_bma2x2->dev_addr,BMA2x2_CHIP_ID_REG, &data_u8, BMA2x2_GEN_READ_WRITE_LENGTH);
		p_bma2x2->chip_id = data_u8;    /* get bit slice */
		/* read the fifo config register and update
		the value to the fifo_config*/
		com_rslt += bma2x2_read_reg(BMA2x2_FIFO_MODE_REG,&config_data_u8, BMA2x2_GEN_READ_WRITE_LENGTH);
...
u8 *data_u8, u8 len_u8)
{
	...
	if (p_bma2x2 == BMA2x2_NULL) {
		...
		return E_BMA2x2_NULL_PTR;
			...
			/*Read the data from the register*/
			com_rslt = p_bma2x2->BMA2x2_BUS_READ_FUNC(p_bma2x2->dev_addr, adr_u8, data_u8, len_u8);
		}
	return com_rslt;
}
/*!
...
{
	BMA2x2_RETURN_FUNCTION_TYPE com_rslt = BMA_SUCCESS;
 ...
 *	@retval 0 -> Success
 *	@retval -1 -> BMA_ERROR
 *	@retval -127 -> Null Pointer BMA_ERROR
 *
*/
